...
// ===================== LOCAL STORAGE CACHE (INSTANT) =====================
function getLocalProducts() {
  try {
    var data = localStorage.getItem('cachedProducts');
...
async function syncProductsIfNeeded() {
  var now = Date.now();
  if (now - lastSyncTime < SYNC_INTERVAL && getLocalProducts()) {
    return getLocalProducts();
  }
  
  // Only sync if cache is old or doesn't exist
  try {
    ...
    return result.data || getLocalProducts() || [];
  } catch(e) {
...
async function refreshProductList() {
  ...
  if (cached) {
    ...
    var start = 0, end = Math.min(productPageSize, cached.length);
    renderProductTable(cached.slice(start, end));
  }
  ...
  syncProductsIfNeeded().then(function(fresh) {
    if (fresh && (!cached || fresh.length !== cached.length)) {
      ...
      var start = 0, end = Math.min(productPageSize, fresh.length);
      renderProductTable(fresh.slice(start, end));
    }
  });
}
...
async function loadProductPage() {
  var cached = getLocalProducts();
  if (!cached) {
    cached = await syncProductsIfNeeded();
  }
  totalProducts = cached.length;
  var start = (productPage - 1) * productPageSize;
  var end = Math.min(start + productPageSize, cached.length);
  renderProductTable(cached.slice(start, end));
}
...
function filterProductList() {
  ...
  filterTimer = setTimeout(function() {
    var query = document.getElementById('invSearch') ? document.getElementById('invSearch').value.trim().toLowerCase() : '';
    var cached = getLocalProducts();
    if (!cached) { refreshProductList(); return; }
    
    if (!query) {
      totalProducts = cached.length;
      var s = 0, e = Math.min(productPageSize, cached.length);
      renderProductTable(cached.slice(s, e));
      return;
    }
    
    // Search in cached data (instant!)
    var filtered = cached.filter(function(p) {
      return (p.nama && p.nama.toLowerCase().indexOf(query) !== -1) ||
             (p.barcode && p.barcode.toLowerCase().indexOf(query) !== -1) ||
             (p.kategori && p.kategori.toLowerCase().indexOf(query) !== -1);
    });
    
    totalProducts = filtered.length;
    renderProductTable(filtered.slice(0, 100));
  }, 100); // 100ms debounce (was 300ms)
}
...
async function hapusProdukDariDaftar(b) {
  ...
  // 1. Remove from local cache IMMEDIATELY (instant UI update)
  var cached = getLocalProducts() || [];
  cached = cached.filter(function(p) { return p.barcode !== b; });
  setLocalProducts(cached);
  totalProducts = cached.length;
  loadProductPage(); // Instant update from cache
  
  // 2. Delete from Supabase in background
  try {
    await supabaseClient.from('products').delete().eq('barcode', b);
    lastSyncTime = 0; // Force re-sync next time
  } catch(e) {
    // If delete fails, re-sync
    alert('Gagal menghapus, menyegarkan data...');
    localStorage.removeItem('cachedProducts');
    refreshProductList();
  }
}

// ===================== INSTANT SAVE (Optimistic) =====================
// Update the save handler in isiFormProduk to update local cache immediately
// The existing code calls upsertProduct which goes to Supabase
// We add cache update after successful save

// Find the save button onclick and add after upsertProduct success:
// Update local cache
async function updateLocalProduct(product) {
  var cached = getLocalProducts() || [];
  var found = false;
  for (var i = 0; i < cached.length; i++) {
    if (cached[i].barcode === product.barcode) {
      cached[i] = product;
      found = true;
      break;
    }
...
// ===================== REST OF INVENTORY (unchanged fast parts) =====================
...
// Label dialog functions (unchanged from before)
function updateLabelDialogStatus() {
  ...
  if (led) led.className = 'led ' + (c ? 'led-green' : 'led-red'); if (txt) txt.textContent = c ? 'Label printer terhubung' : 'Label printer tidak terhubung';
}
