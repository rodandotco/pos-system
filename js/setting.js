...
function bukaPopupTunai() {
  ...
  modal.innerHTML = '<div style="background:#fff;padding:20px;border-radius:8px;width:320px;text-align:center;"><h3>Pembayaran Tunai</h3><div id="popupNominalGrid" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px;">' + [100000,50000,20000,10000,5000,2000,1000,500,200].map(n => '<button class="nominal-btn-popup" onclick="tambahNominalPopup(' + n + ')">Rp ' + n.toLocaleString('id') + '</button>').join('') + '</div><input type="number" id="inputBayarPopup" value="' + bayarValue + '" placeholder="0" style="width:100%;padding:10px;font-size:18px;box-sizing:border-box;text-align:right;" onfocus="this.select()"><div style="margin-top:10px;"><button id="btnSimpanTunai" class="btn-sm">Simpan</button><button id="btnBatalTunai" class="btn-sm btn-danger">Batal</button></div></div>';
...
function renderCart() {
  ...
  cart.forEach((item, idx) => {
    ...
    row.innerHTML = '<td>' + item.nama + '</td>' +
      '<td>' + (item.isGrosir ? '<span style="color:#e53935;font-size:11px;font-weight:bold;">HARGA GROSIR</span><br>' : '') + 'Rp' + item.harga.toLocaleString('id') + (item.isGrosir ? '<br><small style="color:#999;text-decoration:line-through;">Rp' + item.hargaAsli.toLocaleString('id') + '</small>' : '') + '</td>' +
      '<td><div class="qty-control"><button onclick="changeQty(' + idx + ',-1)">−</button><input type="number" min="1" value="' + item.qty + '" onchange="updateQty(' + idx + ',this.value)" style="width:60px;font-size:16px;text-align:center;padding:5px;"><button onclick="changeQty(' + idx + ',1)">+</button></div></td>' +
      '<td>Rp' + sub.toLocaleString('id') + (diskon > 0 ? '<br><small style="color:#e53935;">Diskon: -Rp' + diskon.toLocaleString('id') + '</small>' : '') + '</td>' +
      '<td>' + (isAdmin ? '<button class="btn-sm" onclick="editDiskonItem(' + idx + ')" title="Diskon">💲</button>' : '') + '<button class="btn-sm" onclick="lihatDetailProduk(\'' + item.barcode + '\')">ℹ️</button><button class="btn-sm btn-danger" onclick="hapusCartItem(' + idx + ')">✕</button></td>';
  });
...
function hitungKembalian() {
  const t = parseInt((document.getElementById('totalCart')?.textContent || '0').replace(/\D/g, '')) || 0;
...
async function bayarDanCetak() {
  ...
  for (let item of cart) {
    const { data: cur } = await supabaseClient.from('products').select('stok').eq('barcode', item.barcode).single();
    if (!cur || cur.stok < item.qty) { alert('Stok "' + item.nama + '" tidak mencukupi!\nTersedia: ' + (cur?.stok || 0) + '\nDiminta: ' + item.qty); return; }
  }
...
async function simpanPesanan() {
  ...
  const s1 = cart.reduce((sum, item) => sum + (item.harga * item.qty) - (item.diskon || 0), 0);
  const gt = s1 - totalDiskonValue;
  ...
  cart = []; totalDiskonValue = 0; bayarValue = 0; updateBayarDisplay(); renderCart();
...
async function muatPesanan(noPesanan) {
  ...
  for (let item of order.items) {
    const p = await getProductByBarcode(item.barcode);
