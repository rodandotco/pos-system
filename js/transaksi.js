...
async function tampilkanPesananTersimpan() {
  ...
  else { listEl.innerHTML = orders.map(o => '<div style="border:1px solid #e0e0e0;border-radius:8px;padding:12px;margin-bottom:8px;"><strong>' + o.no_pesanan + '</strong><br><small>' + new Date(o.created_at).toLocaleString('id-ID') + ' | Staff: ' + (o.created_by || '-') + '</small><br><small>Customer: ' + (o.customer || '-') + ' | Total: <b>Rp' + (o.total||0).toLocaleString('id') + '</b></small><div style="margin-top:4px;font-size:12px;color:#666;">' + o.items.map(i => i.nama + ' x' + i.qty).join(', ') + '</div><div style="margin-top:8px;"><button class="btn-sm" onclick="muatPesanan(\'' + o.no_pesanan + '\')">📥 Muat</button>${(currentUser && currentUser.role === 'admin') ? '<button class="btn-sm btn-danger" onclick="hapusPesanan(\'' + o.no_pesanan + '\')">🗑</button>' : ''}</div></div>').join(''); }
